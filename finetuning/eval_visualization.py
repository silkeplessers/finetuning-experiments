...
def _score_distribution(
    df_scores: pd.DataFrame, charts_dir: Path, label: str, prefix: str, judge_name: str,
) -> None:
...
def _pairwise_chart(
    agg: dict, charts_dir: Path, prefix: str, judge_name: str,
) -> None:
    ...
    for dim, label in [("pairwise_quality", "Language Quality"), ("pairwise_instruction", "Instruction Following")]:
        ...
        if w is not None:
            dims.append((label, w, agg.get(f"{prefix}_{dim}_tie", 0), agg.get(f"{prefix}_{dim}_loss", 0)))
...
def _agreement_heatmap(agg: dict, charts_dir: Path) -> None:
    ...
    for ax, values, title in [(axes[0], agreement, "Agreement Rate"), (axes[1], kappa, "Cohen's Kappa")]:
        ...
        for i, v in enumerate(values):
            ax.text(0, i, f"{v:.2f}", ha="center", va="center", fontsize=10,
...
def generate_charts(
    ...
    for prefix, judge_name in [("j1", "Judge 1"), ("j2", "Judge 2")]:
        ...
        _baseline_vs_finetuned_bars(agg, charts_dir, prefix, judge_name, df_baseline_scores)
